...
async function run() {
    ...
    let dbConnection;
    try {
        dbConnection = await promise_1.default.createConnection({
            host: dbHost,
            user: dbUser,
            password: dbPassword,
            database: dbName,
    ...
    try {
        ...
        await dbConnection.execute(`INSERT INTO organizations (id, name, license_number) 
       VALUES (?, ?, ?) 
       ON DUPLICATE KEY UPDATE name = VALUES(name)`, [demoOrgId, 'WristCare Demo Clinic', 'LIC-DEMO-123']);
        // Active subscription for demo organization
        await dbConnection.execute(`INSERT INTO subscriptions (id, organization_id, status, expires_at)
       VALUES (?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE status = VALUES(status)`, ['demo-sub-1', demoOrgId, 'Active', '2030-01-01 00:00:00']);
        ...
        for (const p of patientsToSeed) {
            // Seed user
            await dbConnection.execute(`INSERT INTO users (id, email, password_hash, role, name)
         VALUES (?, ?, ?, 'patient', ?)
         ON DUPLICATE KEY UPDATE name = VALUES(name)`, [p.userId, p.email, '$2b$12$demoHashNotRealPasswordUsedForSeedingDataPlaceholder', `${p.firstName} ${p.lastName}`]);
            // Seed patient
            await dbConnection.execute(`INSERT INTO patients (id, user_id, organization_id, primary_clinician_id, first_name, last_name, birth_date)
         VALUES (?, ?, ?, NULL, ?, ?, ?)
         ON DUPLICATE KEY UPDATE first_name = VALUES(first_name), last_name = VALUES(last_name)`, [p.id, p.userId, demoOrgId, p.firstName, p.lastName, p.birthDate]);
            ...
            for (const item of metrics) {
                await dbConnection.execute(`INSERT INTO vital_thresholds (id, patient_id, metric, min_value, max_value, duration_seconds)
           VALUES (?, ?, ?, ?, ?, ?)
           ON DUPLICATE KEY UPDATE min_value = VALUES(min_value), max_value = VALUES(max_value)`, [`threshold-${p.id}-${item.metric}`, p.id, item.metric, item.min, item.max, item.duration]);
            }
        }
        console.log('----------------------------------------------------');
