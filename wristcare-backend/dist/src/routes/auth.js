...
router.get('/clinicians', async (req, res) => {
    ...
    try {
        ...
        let [rows] = await pool_1.default.execute(`SELECT c.id, c.user_id, c.organization_id, c.first_name, c.last_name, c.specialty, u.email, u.name
       FROM clinicians c
       ...
       WHERE c.organization_id = ?`, [orgId]);
        ...
        if (rows.length === 0) {
            const [allRows] = await pool_1.default.execute(`SELECT c.id, c.user_id, c.organization_id, c.first_name, c.last_name, c.specialty, u.email, u.name
         FROM clinicians c
         INNER JOIN users u ON c.user_id = u.id`);
            rows = allRows;
        }
...
router.get('/patients', async (req, res) => {
    ...
    try {
        ...
        let [rows] = await pool_1.default.execute(`SELECT p.id, p.user_id, p.organization_id, p.primary_clinician_id, p.first_name, p.last_name, p.birth_date, u.email, u.name,
       ...
       WHERE p.organization_id = ?`, [orgId]);
        ...
        if (rows.length === 0) {
            const [allRows] = await pool_1.default.execute(`SELECT p.id, p.user_id, p.organization_id, p.primary_clinician_id, p.first_name, p.last_name, p.birth_date, u.email, u.name,
                c.first_name AS clinician_first_name, c.last_name AS clinician_last_name
         FROM patients p
         INNER JOIN users u ON p.user_id = u.id
         LEFT JOIN clinicians c ON p.primary_clinician_id = c.id`);
...
router.get('/connections', async (req, res) => {
    ...
    try {
        ...
        if (userRole === 'patient') {
            const [pRows] = await pool_1.default.execute(`SELECT p.*, c.first_name AS clinician_first_name, c.last_name AS clinician_last_name, c.specialty AS clinician_specialty
         FROM patients p
         LEFT JOIN clinicians c ON p.primary_clinician_id = c.id
         WHERE p.user_id = ?`, [userId]);
            const patients = pRows;
            if (patients.length === 0)
                return res.json({ doctor: null, family: [] });
            const patient = patients[0];
            const [fRows] = await pool_1.default.execute(`SELECT fm.id, fm.relationship, u.name, u.email
         FROM family_members fm
         INNER JOIN users u ON fm.user_id = u.id
         WHERE fm.patient_id = ?`, [patient.id]);
            return res.json({
            ...
            const [fmRows] = await pool_1.default.execute(`SELECT fm.id, fm.relationship, p.id AS patient_id, p.first_name, p.last_name, p.birth_date,
                c.first_name AS clinician_first_name, c.last_name AS clinician_last_name
         FROM family_members fm
         ...
         WHERE fm.user_id = ?`, [userId]);
            ...
            const [cRows] = await pool_1.default.execute('SELECT id FROM clinicians WHERE user_id = ?', [userId]);
            const clinicians = cRows;
            if (clinicians.length === 0)
                return res.json({ patients: [] });
            const clinicianId = clinicians[0].id;
            const [patRows] = await pool_1.default.execute(`SELECT p.id, p.first_name, p.last_name, p.birth_date, u.email
         FROM patients p
         INNER JOIN users u ON p.user_id = u.id
         WHERE p.primary_clinician_id = ?`, [clinicianId]);
            const patientsList = patRows;
            for (const pat of patientsList) {
                const [famRows] = await pool_1.default.execute(`SELECT fm.relationship, u.name, u.email
           FROM family_members fm
           INNER JOIN users u ON fm.user_id = u.id
           WHERE fm.patient_id = ?`, [pat.id]);
                pat.family = famRows;
            }
...
router.put('/profile/update', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            if (email) {
                const [existing] = await connection.execute('SELECT id FROM users WHERE email = ? AND id != ?', [email, userId]);
            ...
            if (userRole === 'clinician') {
                ...
                if (patientId && relationship) {
                    ...
                    const familyUUID = crypto_1.default.randomUUID();
                    await connection.execute(`INSERT INTO family_members (id, user_id, patient_id, relationship)
             VALUES (?, ?, ?, ?)
             ON DUPLICATE KEY UPDATE patient_id = VALUES(patient_id), relationship = VALUES(relationship)`, [familyUUID, userId, patientId, relationship]);
                }
                else if (relationship) {
                    await connection.execute('UPDATE family_members SET relationship = ? WHERE user_id = ?', [relationship, userId]);
                }
            }
            await connection.commit();
            const [users] = await pool_1.default.execute('SELECT id, email, name, role FROM users WHERE id = ?', [userId]);
            const user = users[0];
            let organizationId = decoded.organizationId || null;
            let details = {};
            if (user.role === 'clinician') {
                ...
                const [rows] = await pool_1.default.execute(`SELECT fm.*, p.organization_id, p.first_name AS rel_first_name, p.last_name AS rel_last_name
           FROM family_members fm
           INNER JOIN patients p ON fm.patient_id = p.id
